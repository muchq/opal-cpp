...
| `@streaming` blob member | `opal::Blob` + a response writer | Always generates as an `opal::Blob` (Smithy requires `@required`/`@default` on it). When it is the *response* `@httpPayload` of an HTTP-binding operation, the client method also takes a defaulted `const opal::http::BodyWriter&`: pass one and the bytes stream to it with the member left empty, omit it and the payload buffers into the member. Request payloads and the RPC protocols stay buffered — see [production-guide.md](production-guide.md#through-a-generated-client) |
