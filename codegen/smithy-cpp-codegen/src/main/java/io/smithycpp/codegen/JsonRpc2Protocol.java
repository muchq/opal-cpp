 ...
 * object over the shared {@code opal::Document} JSON pivot. No HTTP bindings apply.
 *
...
final class JsonRpc2Protocol implements ProtocolGenerator {
  ...
  public void writeClientHelpers(CppWriter w, CppContext context) {
    ...
    w.write("const opal::Document* error = doc->Find(\"error\");");
    w.write("if (error == nullptr || !error->is_map()) return parsed;");
    w.openBlock(
        "if (const opal::Document* code = error->Find(\"code\"); "
    ...
    w.closeBlock("}");
    w.write(
        "if (const opal::Document* message = error->Find(\"message\"); "
            + "message != nullptr && message->is_string()) parsed.message = message->as_string();");
    w.write(
        "if (const opal::Document* data = error->Find(\"data\"); "
  ...
  public void writeOperationBody(
    ...
    w.write("request.body = opal::json::Encode(opal::Document(std::move(envelope)));");

    ProtocolSupport.writeRpcSend(w, operation);
    w.write("auto envelope_doc = opal::json::Decode(response->body);");
  ...
  public void writeStreamServerRoutes(
    ...
    w.openBlock(
        "(void)stream_router_->Add(\"GET\", \"/\", [handler](const opal::http::HttpRequest&"
            + " request, "
  ...
  private static final ProtocolSupport.ErrorResponseSpec SPEC =
      new ProtocolSupport.ErrorResponseSpec(
          "JsonRpcError", /* errortypeHeader= */ "", ", const opal::Document& id", ", id");
  ...
  public void writeServerHelpers(
    ...
    w.openBlock(
        "opal::http::HttpResponse $L(int code, const std::string& type, "
            + "const std::string& message, opal::DocumentMap data, const opal::Document& id) "
            + "{",
        SPEC.errorFn());
    w.write("if (!type.empty()) data.insert_or_assign(\"__type\", opal::Document(type));");
    ...
    w.closeBlock("}");
    w.write("opal::DocumentMap error;");
    w.write("error.emplace(\"code\", opal::Document(code));");
    w.write("error.emplace(\"message\", opal::Document(std::move(text)));");
    w.write("if (!data.empty()) error.emplace(\"data\", opal::Document(std::move(data)));");
    w.write("opal::DocumentMap envelope;");
    w.write("envelope.emplace(\"jsonrpc\", opal::Document(\"2.0\"));");
    w.write("envelope.emplace(\"error\", opal::Document(std::move(error)));");
    ...
    w.write("response.body = opal::json::Encode(opal::Document(std::move(envelope)));");
  ...
  private void writeOperationDispatch(
    ...
    w.openBlock(
        "opal::http::HttpResponse $L(Handler& handler, const opal::Document& params, "
            + "const opal::Document& id, "
            + ProtocolSupport.REQUEST_CONTEXT_PARAM
            + " context) {",
  ...
  public void writeServerRoutes(
    ...
    w.write(
        "if (const opal::Document* id_doc = decoded->Find(\"id\"); "
    ...
    w.write("const opal::Document empty_params{opal::DocumentMap{}};");
